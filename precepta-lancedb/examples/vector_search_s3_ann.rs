...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let table = db
        .create_table(
            ...
            RecordBatchIterator::new(
                ...
                precepta::new(schema(model.ndims())),
            ),
        )
        .execute()
        .await?;
        ...
        .create_index(
            ...
            lancedb::index::Index::IvfPq(
                IvfPqIndexBuilder::default()
                    ...
                    // Needs to be the same distance type as the one used in sepreceptah params.
    ...
    // Define sepreceptah_params params that will be used by the vector store to perform the vector sepreceptah.
    let sepreceptah_params = SepreceptahParams::default().distance_type(DistanceType::Cosine);

    let vector_store = LanceDbVectorIndex::new(table, model, "id", sepreceptah_params).await?;
