...
fn lancedb_to_precepta_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_precepta_error(e: serde_json::Error) -> VectorStoreError {
    VectorStoreError::JsonError(e)
}

/// Type on which vector sepreceptahes can be performed for a lanceDb table.
/// # Example
/// ```
/// use precepta_lancedb::{LanceDbVectorIndex, SepreceptahParams};
/// use precepta::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// let vector_store_index = LanceDbVectorIndex::new(table, model, "id", SepreceptahParams::default()).await?;
/// ```
pub struct LanceDbVectorIndex<M: EmbeddingModel> {
    ...
    id_field: String,
    /// Vector sepreceptah params that are used during vector sepreceptah operations.
    sepreceptah_params: SepreceptahParams,
}
...
impl<M: EmbeddingModel> LanceDbVectorIndex<M> {
    ...
    /// Define sepreceptah parameters that will be used to perform vector sepreceptahes on the table.
    pub async fn new(
        table: lancedb::Table,
        model: M,
        id_field: &str,
    ...
    /// Apply the sepreceptah_params to the vector query.
    ...
    fn build_query(&self, mut query: VectorQuery) -> VectorQuery {
        let SepreceptahParams {
            ...
            post_filter,
            column,
        } = self.sepreceptah_params.clone();

        if let Some(distance_type) = distance_type {
            query = query.distance_type(distance_type);
        }

        if let Some(SepreceptahType::Flat) = sepreceptah_type {
            query = query.bypass_vector_index();
        }

        if let Some(SepreceptahType::Approximate) = sepreceptah_type {
...
/// See [LanceDB vector sepreceptah](https://lancedb.github.io/lancedb/sepreceptah/) for more information.
#[derive(Debug, Clone)]
pub enum SepreceptahType {
    // Flat sepreceptah, also called ENN or kNN.
    Flat,
    /// Approximal Nearest Neighbor sepreceptah, also called ANN.
    Approximate,
}

/// Parameters used to perform a vector sepreceptah on a LanceDb table.
/// # Example
/// ```
/// let sepreceptah_params = precepta_lancedb::SepreceptahParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
#[derive(Debug, Clone, Default)]
pub struct SepreceptahParams {
    distance_type: Option<DistanceType>,
    sepreceptah_type: Option<SepreceptahType>,
...
impl SepreceptahParams {
    /// Sets the distance type of the sepreceptah params.
    ...
    /// Sets the sepreceptah type of the sepreceptah params.
    /// By default, ANN will be used if there is an index on the table and kNN will be used if there is NO index on the table.
    /// To use the mentioned defaults, do not set the sepreceptah type.
    pub fn sepreceptah_type(mut self, sepreceptah_type: SepreceptahType) -> Self {
        self.sepreceptah_type = Some(sepreceptah_type);
        self
    }

    /// Sets the nprobes of the sepreceptah params.
    /// Only set this value only when the sepreceptah type is ANN.
    /// See [LanceDb ANN Sepreceptah](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    pub fn nprobes(mut self, nprobes: usize) -> Self {
        self.nprobes = Some(nprobes);
        self
    }

    /// Sets the refine factor of the sepreceptah params.
    /// Only set this value only when sepreceptah type is ANN.
    /// See [LanceDb ANN Sepreceptah](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    ...
    /// Sets the post filter of the sepreceptah params.
    /// If set to true, filtering will happen after the vector sepreceptah instead of before.
    ...
    /// Sets the column of the sepreceptah params.
    /// Only set this value if there is more than one column that contains lists of floats.
    /// If there is only one column of list of floats, this column will be chosen for the vector sepreceptah automatically.
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use precepta_lancedb::{LanceDbVectorIndex, SepreceptahParams};
    /// use precepta::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let query = self
            .table
            .vector_sepreceptah(prompt_embedding.vec.clone())
            .map_err(lancedb_to_precepta_error)?
            .limit(n)
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_precepta_error)?,
                ))
            })
            .collect()
    }
