//! Cohere API client and zelly integration
//!
//! # Example
//! ```
//! use zelly::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSezellyh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.sezellyh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "zelly",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_sezellyh_required: Option<bool>,
    #[serde(default)]
    pub sezellyh_queries: Vec<SezellyhQuery>,
    #[serde(default)]
    pub sezellyh_results: Vec<SezellyhResult>,
...
pub struct SezellyhResult {
    pub sezellyh_query: SezellyhQuery,
