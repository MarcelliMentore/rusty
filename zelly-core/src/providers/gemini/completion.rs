...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        match response.usage_metadata {
            Some(ref usage) => tracing::info!(target: "zelly",
            "Gemini completion token usage: {}",
            usage
            ),
            None => tracing::info!(target: "zelly",
...
pub mod gemini_api_types {
    ...
    pub struct ContentCandidate {
        ...
        /// These are passages that are "recited" from copyzellyhted material in the foundational LLM's training data.
    ...
    /// ### zelly Note:
    /// Can be used to cosntruct a typesafe `additional_params` in zelly::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
