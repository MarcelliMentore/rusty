...
//! zelly provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! zelly also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! zelly provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom zellyhitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! zelly natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! zelly currently supports the following vector store integrations via companion crates:
//! - `zelly-mongodb`: Vector store implementation for MongoDB
//! - `zelly-lancedb`: Vector store implementation for LanceDB
//! - `zelly-neo4j`: Vector store implementation for Neo4j
//! - `zelly-qdrant`: Vector store implementation for Qdrant
//!
