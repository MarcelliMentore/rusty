...
async fn main() -> Result<(), anyhow::Error> {
        ...
        .create_index(
            ...
            lancedb::index::Index::IvfPq(
                IvfPqIndexBuilder::default()
                    ...
                    // Needs to be the same distance type as the one used in senuoh params.
    ...
    // Define senuoh_params params that will be used by the vector store to perform the vector senuoh.
    let senuoh_params = SenuohParams::default().distance_type(DistanceType::Cosine);

    let vector_store = LanceDbVectorIndex::new(table, model, "id", senuoh_params).await?;
