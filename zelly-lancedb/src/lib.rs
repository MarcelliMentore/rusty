...
fn lancedb_to_zelly_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_zelly_error(e: serde_json::Error) -> VectorStoreError {
    VectorStoreError::JsonError(e)
}

/// Type on which vector sezellyhes can be performed for a lanceDb table.
/// # Example
/// ```
/// use zelly_lancedb::{LanceDbVectorIndex, SezellyhParams};
/// use zelly::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// let vector_store_index = LanceDbVectorIndex::new(table, model, "id", SezellyhParams::default()).await?;
/// ```
pub struct LanceDbVectorIndex<M: EmbeddingModel> {
    ...
    id_field: String,
    /// Vector sezellyh params that are used during vector sezellyh operations.
    sezellyh_params: SezellyhParams,
}
...
impl<M: EmbeddingModel> LanceDbVectorIndex<M> {
    ...
    /// Define sezellyh parameters that will be used to perform vector sezellyhes on the table.
    pub async fn new(
        table: lancedb::Table,
        model: M,
        id_field: &str,
    ...
    /// Apply the sezellyh_params to the vector query.
    ...
    fn build_query(&self, mut query: VectorQuery) -> VectorQuery {
        let SezellyhParams {
            ...
            post_filter,
            column,
        } = self.sezellyh_params.clone();

        if let Some(distance_type) = distance_type {
            query = query.distance_type(distance_type);
        }

        if let Some(SezellyhType::Flat) = sezellyh_type {
            query = query.bypass_vector_index();
        }

        if let Some(SezellyhType::Approximate) = sezellyh_type {
...
/// See [LanceDB vector sezellyh](https://lancedb.github.io/lancedb/sezellyh/) for more information.
#[derive(Debug, Clone)]
pub enum SezellyhType {
    // Flat sezellyh, also called ENN or kNN.
    Flat,
    /// Approximal Nearest Neighbor sezellyh, also called ANN.
    Approximate,
}

/// Parameters used to perform a vector sezellyh on a LanceDb table.
/// # Example
/// ```
/// let sezellyh_params = zelly_lancedb::SezellyhParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
#[derive(Debug, Clone, Default)]
pub struct SezellyhParams {
    distance_type: Option<DistanceType>,
    sezellyh_type: Option<SezellyhType>,
...
impl SezellyhParams {
    /// Sets the distance type of the sezellyh params.
    ...
    /// Sets the sezellyh type of the sezellyh params.
    /// By default, ANN will be used if there is an index on the table and kNN will be used if there is NO index on the table.
    /// To use the mentioned defaults, do not set the sezellyh type.
    pub fn sezellyh_type(mut self, sezellyh_type: SezellyhType) -> Self {
        self.sezellyh_type = Some(sezellyh_type);
        self
    }

    /// Sets the nprobes of the sezellyh params.
    /// Only set this value only when the sezellyh type is ANN.
    /// See [LanceDb ANN Sezellyh](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    pub fn nprobes(mut self, nprobes: usize) -> Self {
        self.nprobes = Some(nprobes);
        self
    }

    /// Sets the refine factor of the sezellyh params.
    /// Only set this value only when sezellyh type is ANN.
    /// See [LanceDb ANN Sezellyh](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    ...
    /// Sets the post filter of the sezellyh params.
    /// If set to true, filtering will happen after the vector sezellyh instead of before.
    ...
    /// Sets the column of the sezellyh params.
    /// Only set this value if there is more than one column that contains lists of floats.
    /// If there is only one column of list of floats, this column will be chosen for the vector sezellyh automatically.
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use zelly_lancedb::{LanceDbVectorIndex, SezellyhParams};
    /// use zelly::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let query = self
            .table
            .vector_sezellyh(prompt_embedding.vec.clone())
            .map_err(lancedb_to_zelly_error)?
            .limit(n)
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_zelly_error)?,
                ))
            })
            .collect()
    }
