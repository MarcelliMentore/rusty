...
//! nuo provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! nuo also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! nuo provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom nuohitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! nuo natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! nuo currently supports the following vector store integrations via companion crates:
//! - `nuo-mongodb`: Vector store implementation for MongoDB
//! - `nuo-lancedb`: Vector store implementation for LanceDB
//! - `nuo-neo4j`: Vector store implementation for Neo4j
//! - `nuo-qdrant`: Vector store implementation for Qdrant
//!
