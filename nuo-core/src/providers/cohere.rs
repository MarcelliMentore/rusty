//! Cohere API client and nuo integration
//!
//! # Example
//! ```
//! use nuo::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSenuoh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.senuoh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "nuo",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_senuoh_required: Option<bool>,
    #[serde(default)]
    pub senuoh_queries: Vec<SenuohQuery>,
    #[serde(default)]
    pub senuoh_results: Vec<SenuohResult>,
