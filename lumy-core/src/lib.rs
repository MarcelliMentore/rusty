//! lumy is a Rust library for building LLM-powered applications that focuses on ergonomics and modularity.
//!
//! # Table of contents
...
//! lumy provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! lumy also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! lumy provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom lumyhitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! lumy natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! lumy currently supports the following vector store integrations via companion crates:
//! - `lumy-mongodb`: Vector store implementation for MongoDB
//! - `lumy-lancedb`: Vector store implementation for LanceDB
//! - `lumy-neo4j`: Vector store implementation for Neo4j
//! - `lumy-qdrant`: Vector store implementation for Qdrant
//!
