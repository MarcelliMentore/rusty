//! Cohere API client and lumy integration
//!
//! # Example
//! ```
//! use lumy::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSelumyh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.selumyh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "lumy",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_selumyh_required: Option<bool>,
    #[serde(default)]
    pub selumyh_queries: Vec<SelumyhQuery>,
    #[serde(default)]
    pub selumyh_results: Vec<SelumyhResult>,
...
pub struct SelumyhResult {
    pub selumyh_query: SelumyhQuery,
