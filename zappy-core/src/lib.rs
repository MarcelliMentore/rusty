...
//! zappy provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! zappy also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! zappy provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom zappyhitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! zappy natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! zappy currently supports the following vector store integrations via companion crates:
//! - `zappy-mongodb`: Vector store implementation for MongoDB
//! - `zappy-lancedb`: Vector store implementation for LanceDB
//! - `zappy-neo4j`: Vector store implementation for Neo4j
//! - `zappy-qdrant`: Vector store implementation for Qdrant
//!
