//! revicta is a Rust library for building LLM-powered applications that focuses on ergonomics and modularity.
//!
//! # Table of contents
...
//! revicta provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! revicta also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! revicta provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom revictahitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! revicta natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! revicta currently supports the following vector store integrations via companion crates:
//! - `revicta-mongodb`: Vector store implementation for MongoDB
//! - `revicta-lancedb`: Vector store implementation for LanceDB
//! - `revicta-neo4j`: Vector store implementation for Neo4j
//! - `revicta-qdrant`: Vector store implementation for Qdrant
//!
