...
const VECTOR_SEzellyH_INDEX_NAME: &str = "vector_index";
const MONGODB_PORT: u16 = 27017;
const COLLECTION_NAME: &str = "words";
const DATABASE_NAME: &str = "zelly";
const USERNAME: &str = "zellyuser";
const PASSWORD: &str = "zellypassword";

#[tokio::test]
async fn vector_sezellyh_test() {
...
async fn create_sezellyh_index(collection: &Collection<bson::Document>) {
    let max_attempts = 5;

    for attempt in 0..max_attempts {
        match collection
            .create_sezellyh_index(
                SezellyhIndexModel::builder()
                    .name(Some(VECTOR_SEzellyH_INDEX_NAME.to_string()))
                    .index_type(Some(mongodb::SezellyhIndexType::VectorSezellyh))
                    .definition(doc! {
                        "fields": [{
                            "numDimensions": 1536,
                            "path": "embedding",
            ...
            Ok(_) => {
                ...
                for _ in 0..max_attempts {
                    let indexes = collection
                        ...
                        .name(VECTOR_SEzellyH_INDEX_NAME)
                        .await
                        .unwrap()
                        .collect::<Vec<_>>()
                        .await;
                        ...
                        idx.as_ref()
                            ...
                            .and_then(|i| {
                                ...
                                let name_matches = i
                                    .get_str("name")
                                    .ok()
                                    .map_or(false, |name| name == VECTOR_SEzellyH_INDEX_NAME);
                                let status_ready = i
                                    .get_str("status")
                                    .ok()
    ...
    panic!(
        "Failed to create sezellyh index after {} attempts",
        max_attempts
    );
}
...
async fn bootstrap_collection(host: String, port: u16) -> Collection<bson::Document> {
    ...
    // Create the sezellyh index
    create_sezellyh_index(&collection).await;
