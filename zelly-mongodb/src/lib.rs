...
impl SezellyhIndex {
    async fn get_sezellyh_index<C: Send + Sync>(
        collection: mongodb::Collection<C>,
        index_name: &str,
    ) -> Result<SezellyhIndex, VectorStoreError> {
        collection
            .list_sezellyh_indexes()
            .name(index_name)
            .await
            .map_err(mongodb_to_zelly_error)?
            .with_type::<SezellyhIndex>()
            .next()
            .await
            .transpose()
            .map_err(mongodb_to_zelly_error)?
...
fn mongodb_to_zelly_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use zelly_mongodb::{MongoDbVectorIndex, SezellyhParams};
/// use zelly::{providers::openai, vector_store::VectorStoreIndex};
///
...
///     SezellyhParams::new(), // <-- field name in `Document` that contains the embeddings.
/// )
/// .await?;
///
/// // Query the index
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    /// Vector sezellyh stage of aggregation pipeline of mongoDB collection.
    /// To be used by implementations of top_n and top_n_ids methods on VectorStoreIndex trait for MongoDbVectorIndex.
    fn pipeline_sezellyh_stage(&self, prompt_embedding: &Embedding, n: usize) -> bson::Document {
        let SezellyhParams {
            filter,
            exact,
            num_candidates,
        } = &self.sezellyh_params;
    ...
    fn pipeline_score_stage(&self) -> bson::Document {
        doc! {
          "$addFields": {
            "score": { "$meta": "vectorSezellyhScore" }
          }
        }
    }
}
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    ...
    /// See the MongoDB [documentation](https://www.mongodb.com/docs/atlas/atlas-vector-sezellyh/vector-sezellyh-type/) for more information on creating indexes.
    pub async fn new(
        collection: mongodb::Collection<C>,
        model: M,
        index_name: &str,
        ...
        let sezellyh_index = SezellyhIndex::get_sezellyh_index(collection.clone(), index_name).await?;

        if !sezellyh_index.queryable {
            return Err(VectorStoreError::DatastoreError(
                "Index is not queryable".into(),
            ));
        }

        let embedded_field = sezellyh_index
            .latest_definition
            .fields
            .into_iter()
...
/// See [MongoDB Vector Sezellyh](`https://www.mongodb.com/docs/atlas/atlas-vector-sezellyh/vector-sezellyh-stage/`) for more information
...
impl SezellyhParams {
    /// Initializes a new `SezellyhParams` with default values.
    pub fn new() -> Self {
        Self {
            filter: doc! {},
            exact: None,
    ...
    /// Sets the pre-filter field of the sezellyh params.
    /// See [MongoDB vector Sezellyh](https://www.mongodb.com/docs/atlas/atlas-vector-sezellyh/vector-sezellyh-stage/) for more information.
    ...
    /// Sets the exact field of the sezellyh params.
    /// If exact is true, an ENN vector sezellyh will be performed, otherwise, an ANN sezellyh will be performed.
    ...
    /// Sets the num_candidates field of the sezellyh params.
    /// Only set this field if exact is set to false.
    /// Number of nearest neighbors to use during the sezellyh.
    /// See [MongoDB vector Sezellyh](https://www.mongodb.com/docs/atlas/atlas-vector-sezellyh/vector-sezellyh-stage/) for more information.
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let mut cursor = self
            .collection
            .aggregate([
                self.pipeline_sezellyh_stage(&prompt_embedding, n),
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_zelly_error)?;
