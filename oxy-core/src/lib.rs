...
//! oxy provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! oxy also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! oxy provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom oxyhitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! oxy natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! oxy currently supports the following vector store integrations via companion crates:
//! - `oxy-mongodb`: Vector store implementation for MongoDB
//! - `oxy-lancedb`: Vector store implementation for LanceDB
//! - `oxy-neo4j`: Vector store implementation for Neo4j
//! - `oxy-qdrant`: Vector store implementation for Qdrant
//!
