//! Cohere API client and oxy integration
//!
//! # Example
//! ```
//! use oxy::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSeoxyh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.seoxyh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "oxy",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_seoxyh_required: Option<bool>,
    #[serde(default)]
    pub seoxyh_queries: Vec<SeoxyhQuery>,
    #[serde(default)]
    pub seoxyh_results: Vec<SeoxyhResult>,
