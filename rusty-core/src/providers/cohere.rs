//! Cohere API client and rusty integration
//!
//! # Example
//! ```
//! use rusty::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSerustyh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.serustyh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "rusty",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_serustyh_required: Option<bool>,
    #[serde(default)]
    pub serustyh_queries: Vec<SerustyhQuery>,
    #[serde(default)]
    pub serustyh_results: Vec<SerustyhResult>,
...
pub struct SerustyhResult {
    pub serustyh_query: SerustyhQuery,
