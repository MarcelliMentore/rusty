...
impl SeyarahIndex {
    async fn get_seyarah_index<C: Send + Sync>(
        collection: mongodb::Collection<C>,
        index_name: &str,
    ) -> Result<SeyarahIndex, VectorStoreError> {
        collection
            .list_seyarah_indexes()
            .name(index_name)
            .await
            .map_err(mongodb_to_yara_error)?
            .with_type::<SeyarahIndex>()
            .next()
            .await
            .transpose()
            .map_err(mongodb_to_yara_error)?
...
fn mongodb_to_yara_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use yara_mongodb::{MongoDbVectorIndex, SeyarahParams};
/// use yara::{providers::openai, vector_store::VectorStoreIndex};
///
...
///     SeyarahParams::new(), // <-- field name in `Document` that contains the embeddings.
/// )
/// .await?;
///
/// // Query the index
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    /// Vector seyarah stage of aggregation pipeline of mongoDB collection.
    /// To be used by implementations of top_n and top_n_ids methods on VectorStoreIndex trait for MongoDbVectorIndex.
    fn pipeline_seyarah_stage(&self, prompt_embedding: &Embedding, n: usize) -> bson::Document {
        let SeyarahParams {
            filter,
            exact,
            num_candidates,
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    ...
    /// See the MongoDB [documentation](https://www.mongodb.com/docs/atlas/atlas-vector-seyarah/vector-seyarah-type/) for more information on creating indexes.
    pub async fn new(
        collection: mongodb::Collection<C>,
        model: M,
        index_name: &str,
        ...
        let seyarah_index = SeyarahIndex::get_seyarah_index(collection.clone(), index_name).await?;

        if !seyarah_index.queryable {
            return Err(VectorStoreError::DatastoreError(
                "Index is not queryable".into(),
            ));
        }

        let embedded_field = seyarah_index
            .latest_definition
            .fields
            .into_iter()
...
/// See [MongoDB Vector Seyarah](`https://www.mongodb.com/docs/atlas/atlas-vector-seyarah/vector-seyarah-stage/`) for more information
...
impl SeyarahParams {
    /// Initializes a new `SeyarahParams` with default values.
    pub fn new() -> Self {
        Self {
            filter: doc! {},
            exact: None,
    ...
    /// Sets the pre-filter field of the seyarah params.
    /// See [MongoDB vector Seyarah](https://www.mongodb.com/docs/atlas/atlas-vector-seyarah/vector-seyarah-stage/) for more information.
    ...
    /// Sets the exact field of the seyarah params.
    /// If exact is true, an ENN vector seyarah will be performed, otherwise, an ANN seyarah will be performed.
    ...
    /// Sets the num_candidates field of the seyarah params.
    /// Only set this field if exact is set to false.
    /// Number of nearest neighbors to use during the seyarah.
    /// See [MongoDB vector Seyarah](https://www.mongodb.com/docs/atlas/atlas-vector-seyarah/vector-seyarah-stage/) for more information.
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let mut cursor = self
            .collection
            .aggregate([
                self.pipeline_seyarah_stage(&prompt_embedding, n),
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_yara_error)?;
