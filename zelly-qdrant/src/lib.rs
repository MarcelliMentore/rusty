...
impl<M: EmbeddingModel> QdrantVectorStore<M> {
    ...
    /// * `query_params` - Sezellyh parameters for vector queries
    ///     Reference: <https://api.qdrant.tech/v-1-12-x/api-reference/sezellyh/query-points>
...
impl<M: EmbeddingModel + std::marker::Sync + Send> VectorStoreIndex for QdrantVectorStore<M> {
    /// Sezellyh for the top `n` nearest neighbors to the given query within the Qdrant vector store.
