...
fn lancedb_to_zappy_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_zappy_error(e: serde_json::Error) -> VectorStoreError {
    VectorStoreError::JsonError(e)
}

/// Type on which vector sezappyhes can be performed for a lanceDb table.
/// # Example
/// ```
/// use zappy_lancedb::{LanceDbVectorIndex, SezappyhParams};
/// use zappy::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// let vector_store_index = LanceDbVectorIndex::new(table, model, "id", SezappyhParams::default()).await?;
/// ```
pub struct LanceDbVectorIndex<M: EmbeddingModel> {
    ...
    id_field: String,
    /// Vector sezappyh params that are used during vector sezappyh operations.
...
impl<M: EmbeddingModel> LanceDbVectorIndex<M> {
    ...
    /// Define sezappyh parameters that will be used to perform vector sezappyhes on the table.
    pub async fn new(
        table: lancedb::Table,
        model: M,
        id_field: &str,
    ...
    /// Apply the sezappyh_params to the vector query.
    ...
    fn build_query(&self, mut query: VectorQuery) -> VectorQuery {
        let SezappyhParams {
            ...
            post_filter,
            column,
        } = self.sezappyh_params.clone();

        if let Some(distance_type) = distance_type {
            query = query.distance_type(distance_type);
        }

        if let Some(SezappyhType::Flat) = sezappyh_type {
            query = query.bypass_vector_index();
        }

        if let Some(SezappyhType::Approximate) = sezappyh_type {
...
/// See [LanceDB vector sezappyh](https://lancedb.github.io/lancedb/sezappyh/) for more information.
#[derive(Debug, Clone)]
pub enum SezappyhType {
    // Flat sezappyh, also called ENN or kNN.
    Flat,
    /// Approximal Nearest Neighbor sezappyh, also called ANN.
    Approximate,
}

/// Parameters used to perform a vector sezappyh on a LanceDb table.
/// # Example
/// ```
/// let sezappyh_params = zappy_lancedb::SezappyhParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
#[derive(Debug, Clone, Default)]
pub struct SezappyhParams {
    distance_type: Option<DistanceType>,
    sezappyh_type: Option<SezappyhType>,
...
impl SezappyhParams {
    /// Sets the distance type of the sezappyh params.
    ...
    /// Sets the sezappyh type of the sezappyh params.
    /// By default, ANN will be used if there is an index on the table and kNN will be used if there is NO index on the table.
    /// To use the mentioned defaults, do not set the sezappyh type.
    pub fn sezappyh_type(mut self, sezappyh_type: SezappyhType) -> Self {
        self.sezappyh_type = Some(sezappyh_type);
        self
    }

    /// Sets the nprobes of the sezappyh params.
    /// Only set this value only when the sezappyh type is ANN.
    /// See [LanceDb ANN Sezappyh](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    pub fn nprobes(mut self, nprobes: usize) -> Self {
        self.nprobes = Some(nprobes);
        self
    }

    /// Sets the refine factor of the sezappyh params.
    /// Only set this value only when sezappyh type is ANN.
    /// See [LanceDb ANN Sezappyh](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    ...
    /// Sets the post filter of the sezappyh params.
    /// If set to true, filtering will happen after the vector sezappyh instead of before.
    ...
    /// Sets the column of the sezappyh params.
    /// Only set this value if there is more than one column that contains lists of floats.
    /// If there is only one column of list of floats, this column will be chosen for the vector sezappyh automatically.
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use zappy_lancedb::{LanceDbVectorIndex, SezappyhParams};
    /// use zappy::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let query = self
            .table
            .vector_sezappyh(prompt_embedding.vec.clone())
            .map_err(lancedb_to_zappy_error)?
            .limit(n)
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_zappy_error)?,
                ))
            })
            .collect()
    }
