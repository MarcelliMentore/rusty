...
fn lancedb_to_galactica_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_galactica_error(e: serde_json::Error) -> VectorStoreError {
    VectorStoreError::JsonError(e)
}

/// Type on which vector segalacticahes can be performed for a lanceDb table.
/// # Example
/// ```
/// use galactica_lancedb::{LanceDbVectorIndex, SegalacticahParams};
/// use galactica::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// let vector_store_index = LanceDbVectorIndex::new(table, model, "id", SegalacticahParams::default()).await?;
/// ```
pub struct LanceDbVectorIndex<M: EmbeddingModel> {
    ...
    id_field: String,
    /// Vector segalacticah params that are used during vector segalacticah operations.
...
impl<M: EmbeddingModel> LanceDbVectorIndex<M> {
    ...
    /// Define segalacticah parameters that will be used to perform vector segalacticahes on the table.
    pub async fn new(
        table: lancedb::Table,
        model: M,
        id_field: &str,
    ...
    /// Apply the segalacticah_params to the vector query.
    ...
    fn build_query(&self, mut query: VectorQuery) -> VectorQuery {
        let SegalacticahParams {
            ...
            post_filter,
            column,
        } = self.segalacticah_params.clone();

        if let Some(distance_type) = distance_type {
            query = query.distance_type(distance_type);
        }

        if let Some(SegalacticahType::Flat) = segalacticah_type {
            query = query.bypass_vector_index();
        }

        if let Some(SegalacticahType::Approximate) = segalacticah_type {
...
/// See [LanceDB vector segalacticah](https://lancedb.github.io/lancedb/segalacticah/) for more information.
#[derive(Debug, Clone)]
pub enum SegalacticahType {
    // Flat segalacticah, also called ENN or kNN.
    Flat,
    /// Approximal Nearest Neighbor segalacticah, also called ANN.
    Approximate,
}

/// Parameters used to perform a vector segalacticah on a LanceDb table.
/// # Example
/// ```
/// let segalacticah_params = galactica_lancedb::SegalacticahParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
#[derive(Debug, Clone, Default)]
pub struct SegalacticahParams {
    distance_type: Option<DistanceType>,
    segalacticah_type: Option<SegalacticahType>,
...
impl SegalacticahParams {
    /// Sets the distance type of the segalacticah params.
    ...
    /// Sets the segalacticah type of the segalacticah params.
    /// By default, ANN will be used if there is an index on the table and kNN will be used if there is NO index on the table.
    /// To use the mentioned defaults, do not set the segalacticah type.
    pub fn segalacticah_type(mut self, segalacticah_type: SegalacticahType) -> Self {
        self.segalacticah_type = Some(segalacticah_type);
        self
    }

    /// Sets the nprobes of the segalacticah params.
    /// Only set this value only when the segalacticah type is ANN.
    /// See [LanceDb ANN Segalacticah](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    pub fn nprobes(mut self, nprobes: usize) -> Self {
        self.nprobes = Some(nprobes);
        self
    }

    /// Sets the refine factor of the segalacticah params.
    /// Only set this value only when segalacticah type is ANN.
    /// See [LanceDb ANN Segalacticah](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    ...
    /// Sets the post filter of the segalacticah params.
    /// If set to true, filtering will happen after the vector segalacticah instead of before.
    ...
    /// Sets the column of the segalacticah params.
    /// Only set this value if there is more than one column that contains lists of floats.
    /// If there is only one column of list of floats, this column will be chosen for the vector segalacticah automatically.
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use galactica_lancedb::{LanceDbVectorIndex, SegalacticahParams};
    /// use galactica::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let query = self
            .table
            .vector_segalacticah(prompt_embedding.vec.clone())
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_galactica_error)?,
                ))
            })
            .collect()
    }
