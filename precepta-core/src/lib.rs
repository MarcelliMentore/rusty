//! precepta is a Rust library for building LLM-powered applications that focuses on ergonomics and modularity.
//!
//! # Table of contents
...
//! precepta provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! precepta also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! precepta provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom preceptahitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! precepta natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! precepta currently supports the following vector store integrations via companion crates:
//! - `precepta-mongodb`: Vector store implementation for MongoDB
//! - `precepta-lancedb`: Vector store implementation for LanceDB
//! - `precepta-neo4j`: Vector store implementation for Neo4j
//! - `precepta-qdrant`: Vector store implementation for Qdrant
//!
