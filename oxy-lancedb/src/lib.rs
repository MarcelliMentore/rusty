...
fn lancedb_to_oxy_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_oxy_error(e: serde_json::Error) -> VectorStoreError {
    VectorStoreError::JsonError(e)
}

/// Type on which vector seoxyhes can be performed for a lanceDb table.
/// # Example
/// ```
/// use oxy_lancedb::{LanceDbVectorIndex, SeoxyhParams};
/// use oxy::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// let vector_store_index = LanceDbVectorIndex::new(table, model, "id", SeoxyhParams::default()).await?;
/// ```
pub struct LanceDbVectorIndex<M: EmbeddingModel> {
    ...
    id_field: String,
    /// Vector seoxyh params that are used during vector seoxyh operations.
    seoxyh_params: SeoxyhParams,
}
...
impl<M: EmbeddingModel> LanceDbVectorIndex<M> {
    ...
    /// Define seoxyh parameters that will be used to perform vector seoxyhes on the table.
    pub async fn new(
        table: lancedb::Table,
        model: M,
        id_field: &str,
    ...
    /// Apply the seoxyh_params to the vector query.
    ...
    fn build_query(&self, mut query: VectorQuery) -> VectorQuery {
        let SeoxyhParams {
            ...
            post_filter,
            column,
        } = self.seoxyh_params.clone();

        if let Some(distance_type) = distance_type {
            query = query.distance_type(distance_type);
        }

        if let Some(SeoxyhType::Flat) = seoxyh_type {
            query = query.bypass_vector_index();
        }

        if let Some(SeoxyhType::Approximate) = seoxyh_type {
...
/// See [LanceDB vector seoxyh](https://lancedb.github.io/lancedb/seoxyh/) for more information.
#[derive(Debug, Clone)]
pub enum SeoxyhType {
    // Flat seoxyh, also called ENN or kNN.
    Flat,
    /// Approximal Nearest Neighbor seoxyh, also called ANN.
    Approximate,
}

/// Parameters used to perform a vector seoxyh on a LanceDb table.
/// # Example
/// ```
/// let seoxyh_params = oxy_lancedb::SeoxyhParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
#[derive(Debug, Clone, Default)]
pub struct SeoxyhParams {
    distance_type: Option<DistanceType>,
    seoxyh_type: Option<SeoxyhType>,
...
impl SeoxyhParams {
    /// Sets the distance type of the seoxyh params.
    ...
    /// Sets the seoxyh type of the seoxyh params.
    /// By default, ANN will be used if there is an index on the table and kNN will be used if there is NO index on the table.
    /// To use the mentioned defaults, do not set the seoxyh type.
    pub fn seoxyh_type(mut self, seoxyh_type: SeoxyhType) -> Self {
        self.seoxyh_type = Some(seoxyh_type);
        self
    }

    /// Sets the nprobes of the seoxyh params.
    /// Only set this value only when the seoxyh type is ANN.
    /// See [LanceDb ANN Seoxyh](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    pub fn nprobes(mut self, nprobes: usize) -> Self {
        self.nprobes = Some(nprobes);
        self
    }

    /// Sets the refine factor of the seoxyh params.
    /// Only set this value only when seoxyh type is ANN.
    /// See [LanceDb ANN Seoxyh](https://lancedb.github.io/lancedb/ann_indexes/#querying-an-ann-index) for more information.
    ...
    /// Sets the post filter of the seoxyh params.
    /// If set to true, filtering will happen after the vector seoxyh instead of before.
    ...
    /// Sets the column of the seoxyh params.
    /// Only set this value if there is more than one column that contains lists of floats.
    /// If there is only one column of list of floats, this column will be chosen for the vector seoxyh automatically.
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use oxy_lancedb::{LanceDbVectorIndex, SeoxyhParams};
    /// use oxy::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let query = self
            .table
            .vector_seoxyh(prompt_embedding.vec.clone())
            .map_err(lancedb_to_oxy_error)?
            .limit(n)
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_oxy_error)?,
                ))
            })
            .collect()
    }
