...
impl SerustyhIndex {
    async fn get_serustyh_index<C: Send + Sync>(
        collection: mongodb::Collection<C>,
        index_name: &str,
    ) -> Result<SerustyhIndex, VectorStoreError> {
        collection
            .list_serustyh_indexes()
            .name(index_name)
            .await
            .map_err(mongodb_to_rusty_error)?
            .with_type::<SerustyhIndex>()
            .next()
            .await
            .transpose()
            .map_err(mongodb_to_rusty_error)?
...
fn mongodb_to_rusty_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use rusty_mongodb::{MongoDbVectorIndex, SerustyhParams};
/// use rusty::{providers::openai, vector_store::VectorStoreIndex};
///
...
///     SerustyhParams::new(), // <-- field name in `Document` that contains the embeddings.
/// )
/// .await?;
///
/// // Query the index
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    /// Vector serustyh stage of aggregation pipeline of mongoDB collection.
    /// To be used by implementations of top_n and top_n_ids methods on VectorStoreIndex trait for MongoDbVectorIndex.
    fn pipeline_serustyh_stage(&self, prompt_embedding: &Embedding, n: usize) -> bson::Document {
        let SerustyhParams {
            filter,
            exact,
            num_candidates,
        } = &self.serustyh_params;
    ...
    fn pipeline_score_stage(&self) -> bson::Document {
        doc! {
          "$addFields": {
            "score": { "$meta": "vectorSerustyhScore" }
          }
        }
    }
}
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    ...
    /// See the MongoDB [documentation](https://www.mongodb.com/docs/atlas/atlas-vector-serustyh/vector-serustyh-type/) for more information on creating indexes.
    pub async fn new(
        collection: mongodb::Collection<C>,
        model: M,
        index_name: &str,
        ...
        let serustyh_index = SerustyhIndex::get_serustyh_index(collection.clone(), index_name).await?;

        if !serustyh_index.queryable {
            return Err(VectorStoreError::DatastoreError(
                "Index is not queryable".into(),
            ));
        }

        let embedded_field = serustyh_index
            .latest_definition
            .fields
            .into_iter()
...
/// See [MongoDB Vector Serustyh](`https://www.mongodb.com/docs/atlas/atlas-vector-serustyh/vector-serustyh-stage/`) for more information
...
impl SerustyhParams {
    /// Initializes a new `SerustyhParams` with default values.
    pub fn new() -> Self {
        Self {
            filter: doc! {},
            exact: None,
    ...
    /// Sets the pre-filter field of the serustyh params.
    /// See [MongoDB vector Serustyh](https://www.mongodb.com/docs/atlas/atlas-vector-serustyh/vector-serustyh-stage/) for more information.
    ...
    /// Sets the exact field of the serustyh params.
    /// If exact is true, an ENN vector serustyh will be performed, otherwise, an ANN serustyh will be performed.
    ...
    /// Sets the num_candidates field of the serustyh params.
    /// Only set this field if exact is set to false.
    /// Number of nearest neighbors to use during the serustyh.
    /// See [MongoDB vector Serustyh](https://www.mongodb.com/docs/atlas/atlas-vector-serustyh/vector-serustyh-stage/) for more information.
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let mut cursor = self
            .collection
            .aggregate([
                self.pipeline_serustyh_stage(&prompt_embedding, n),
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_rusty_error)?;
