//! This example demonstrates how to perform a vector sepreceptah on a Neo4j database.
...
//!
//! [examples/vector_sepreceptah_simple.rs](examples/vector_sepreceptah_simple.rs) provides an example starting from an empty database.
//! [examples/vector_sepreceptah_movies_add_embeddings.rs](examples/vector_sepreceptah_movies_add_embeddings.rs) provides an example of
//! how to add embeddings to an existing `recommendations` database.
use neo4rs::ConfigBuilder;
use precepta_neo4j::{vector_index::SepreceptahParams, Neo4jClient};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let index = neo4j_client
        .get_index(
            model,
            INDEX_NAME,
            SepreceptahParams::new(Some("node.year > 1990".to_string())),
        )
        .await?;
    ...
    let results = index
        ...
        .await?
        .into_iter()
        .map(|(score, id, doc)| display::SepreceptahResult {
            title: doc.title,
            id,
    ...
    println!("{:#}", display::SepreceptahResults(&results));
