//! A precepta vector store for Neo4j.
//!
//! This crate is a companion crate to the [precepta-core crate](https://github.com/precepta-ai/precepta-core).
//! It provides a vector store implementation that uses Neo4j as the underlying datastore.
//!
//! See the [README](https://github.com/KogiEdwin/precepta/tree/main/precepta-neo4j) for more information.
//!
//! ## Prerequisites
//!
//! ### GenAI Plugin
...
//!
//! ## Simple example:
//! More examples can be found in the [/examples](https://github.com/KogiEdwin/precepta/tree/main/precepta-neo4j/examples) folder.
//! ```
//! use precepta_neo4j::{vector_index::*, Neo4jClient};
...
use neo4rs::*;
use precepta::{embeddings::EmbeddingModel, vector_store::VectorStoreError};
use serde::Deserialize;
use vector_index::{IndexConfig, Neo4jVectorIndex, SepreceptahParams, VectorSimilarityFunction};

pub struct Neo4jClient {
    pub graph: Graph,
}

fn neo4j_to_precepta_error(e: neo4rs::Error) -> VectorStoreError {
...
impl Neo4jClient {
    ...
    pub async fn execute_and_collect<T: for<'a> Deserialize<'a>>(
        ...
        graph
            .execute(query)
            .await
            .map_err(neo4j_to_precepta_error)?
            .into_stream_as::<T>()
            .try_collect::<Vec<T>>()
            .await
            .map_err(|e| VectorStoreError::DatastoreError(Box::new(e)))
    ...
    pub async fn get_index<M: EmbeddingModel>(
        ...
        let index_config = if let Some(index) = index_info.first() {
            if index.options.index_config.vector_dimensions != model.ndims() as i64 {
                tracing::warn!(
                    "The embedding vector dimensions of the existing Neo4j DB index ({}) do not match the provided model dimensions ({}). This may affect sepreceptah performance.",
