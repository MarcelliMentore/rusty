...
async fn main() -> Result<(), anyhow::Error> {
        ...
        .create_index(
            ...
            lancedb::index::Index::IvfPq(
                IvfPqIndexBuilder::default()
                    ...
                    // Needs to be the same distance type as the one used in seyarah params.
    ...
    // Define seyarah_params params that will be used by the vector store to perform the vector seyarah.
    let seyarah_params = SeyarahParams::default().distance_type(DistanceType::Cosine);

    let vector_store = LanceDbVectorIndex::new(table, model, "id", seyarah_params).await?;
