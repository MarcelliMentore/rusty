...
use mongodb::{
    ...
    Collection, SerevictahIndexModel,
};
use revicta::{
...
const VECTOR_SErevictaH_INDEX_NAME: &str = "vector_index";
const MONGODB_PORT: u16 = 27017;
const COLLECTION_NAME: &str = "words";
const DATABASE_NAME: &str = "revicta";
const USERNAME: &str = "revictauser";
const PASSWORD: &str = "revictapassword";
...
async fn vector_serevictah_test() {
    ...
    let index = MongoDbVectorIndex::new(
        collection,
        model,
        VECTOR_SErevictaH_INDEX_NAME,
        SerevictahParams::new(),
    )
    .await
    .unwrap();
...
async fn create_serevictah_index(collection: &Collection<bson::Document>) {
    let max_attempts = 5;

    for attempt in 0..max_attempts {
        match collection
            .create_serevictah_index(
                SerevictahIndexModel::builder()
                    .name(Some(VECTOR_SErevictaH_INDEX_NAME.to_string()))
                    .index_type(Some(mongodb::SerevictahIndexType::VectorSerevictah))
                    .definition(doc! {
                        "fields": [{
                            "numDimensions": 1536,
                            "path": "embedding",
            ...
            Ok(_) => {
                ...
                for _ in 0..max_attempts {
                    let indexes = collection
                        ...
                        .name(VECTOR_SErevictaH_INDEX_NAME)
                        .await
                        .unwrap()
                        .collect::<Vec<_>>()
                        .await;
                        ...
                        idx.as_ref()
                            ...
                            .and_then(|i| {
                                ...
                                let name_matches = i
                                    .get_str("name")
                                    .ok()
                                    .map_or(false, |name| name == VECTOR_SErevictaH_INDEX_NAME);
                                let status_ready = i
                                    .get_str("status")
                                    .ok()
    ...
    panic!(
        "Failed to create serevictah index after {} attempts",
        max_attempts
    );
}
...
async fn bootstrap_collection(host: String, port: u16) -> Collection<bson::Document> {
    ...
    // Create the serevictah index
    create_serevictah_index(&collection).await;
