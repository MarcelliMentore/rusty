...
impl SeoxyhIndex {
    async fn get_seoxyh_index<C: Send + Sync>(
        collection: mongodb::Collection<C>,
        index_name: &str,
    ) -> Result<SeoxyhIndex, VectorStoreError> {
        collection
            .list_seoxyh_indexes()
            .name(index_name)
            .await
            .map_err(mongodb_to_oxy_error)?
            .with_type::<SeoxyhIndex>()
            .next()
            .await
            .transpose()
            .map_err(mongodb_to_oxy_error)?
...
fn mongodb_to_oxy_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use oxy_mongodb::{MongoDbVectorIndex, SeoxyhParams};
/// use oxy::{providers::openai, vector_store::VectorStoreIndex};
///
...
///     SeoxyhParams::new(), // <-- field name in `Document` that contains the embeddings.
/// )
/// .await?;
///
/// // Query the index
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    /// Vector seoxyh stage of aggregation pipeline of mongoDB collection.
    /// To be used by implementations of top_n and top_n_ids methods on VectorStoreIndex trait for MongoDbVectorIndex.
    fn pipeline_seoxyh_stage(&self, prompt_embedding: &Embedding, n: usize) -> bson::Document {
        let SeoxyhParams {
            filter,
            exact,
            num_candidates,
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    ...
    /// See the MongoDB [documentation](https://www.mongodb.com/docs/atlas/atlas-vector-seoxyh/vector-seoxyh-type/) for more information on creating indexes.
    pub async fn new(
        collection: mongodb::Collection<C>,
        model: M,
        index_name: &str,
        ...
        let seoxyh_index = SeoxyhIndex::get_seoxyh_index(collection.clone(), index_name).await?;

        if !seoxyh_index.queryable {
            return Err(VectorStoreError::DatastoreError(
                "Index is not queryable".into(),
            ));
        }

        let embedded_field = seoxyh_index
            .latest_definition
            .fields
            .into_iter()
...
/// See [MongoDB Vector Seoxyh](`https://www.mongodb.com/docs/atlas/atlas-vector-seoxyh/vector-seoxyh-stage/`) for more information
...
impl SeoxyhParams {
    /// Initializes a new `SeoxyhParams` with default values.
    pub fn new() -> Self {
        Self {
            filter: doc! {},
            exact: None,
    ...
    /// Sets the pre-filter field of the seoxyh params.
    /// See [MongoDB vector Seoxyh](https://www.mongodb.com/docs/atlas/atlas-vector-seoxyh/vector-seoxyh-stage/) for more information.
    ...
    /// Sets the exact field of the seoxyh params.
    /// If exact is true, an ENN vector seoxyh will be performed, otherwise, an ANN seoxyh will be performed.
    ...
    /// Sets the num_candidates field of the seoxyh params.
    /// Only set this field if exact is set to false.
    /// Number of nearest neighbors to use during the seoxyh.
    /// See [MongoDB vector Seoxyh](https://www.mongodb.com/docs/atlas/atlas-vector-seoxyh/vector-seoxyh-stage/) for more information.
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let mut cursor = self
            .collection
            .aggregate([
                self.pipeline_seoxyh_stage(&prompt_embedding, n),
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_oxy_error)?;
