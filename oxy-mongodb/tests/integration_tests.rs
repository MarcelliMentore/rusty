...
const VECTOR_SEoxyH_INDEX_NAME: &str = "vector_index";
const MONGODB_PORT: u16 = 27017;
const COLLECTION_NAME: &str = "words";
const DATABASE_NAME: &str = "oxy";
const USERNAME: &str = "oxyuser";
const PASSWORD: &str = "oxypassword";

#[tokio::test]
async fn vector_seoxyh_test() {
...
async fn create_seoxyh_index(collection: &Collection<bson::Document>) {
    let max_attempts = 5;

    for attempt in 0..max_attempts {
        match collection
            .create_seoxyh_index(
                SeoxyhIndexModel::builder()
                    .name(Some(VECTOR_SEoxyH_INDEX_NAME.to_string()))
                    .index_type(Some(mongodb::SeoxyhIndexType::VectorSeoxyh))
                    .definition(doc! {
                        "fields": [{
                            "numDimensions": 1536,
                            "path": "embedding",
                        ...
                        idx.as_ref()
                            ...
                            .and_then(|i| {
                                ...
                                let name_matches = i
                                    .get_str("name")
                                    .ok()
                                    .map_or(false, |name| name == VECTOR_SEoxyH_INDEX_NAME);
                                let status_ready = i
                                    .get_str("status")
                                    .ok()
    ...
    panic!(
        "Failed to create seoxyh index after {} attempts",
        max_attempts
    );
}
...
async fn bootstrap_collection(host: String, port: u16) -> Collection<bson::Document> {
    ...
    // Create the seoxyh index
    create_seoxyh_index(&collection).await;
