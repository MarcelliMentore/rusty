//! Cohere API client and yara integration
//!
//! # Example
//! ```
//! use yara::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSeyarah units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.seyarah_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "yara",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_seyarah_required: Option<bool>,
    #[serde(default)]
    pub seyarah_queries: Vec<SeyarahQuery>,
    #[serde(default)]
    pub seyarah_results: Vec<SeyarahResult>,
...
pub struct SeyarahResult {
    pub seyarah_query: SeyarahQuery,
