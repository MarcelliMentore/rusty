...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        match response.usage_metadata {
            Some(ref usage) => tracing::info!(target: "yara",
            "Gemini completion token usage: {}",
            usage
            ),
            None => tracing::info!(target: "yara",
...
pub mod gemini_api_types {
    ...
    pub struct ContentCandidate {
        ...
        /// These are passages that are "recited" from copyyarahted material in the foundational LLM's training data.
    ...
    /// ### yara Note:
    /// Can be used to cosntruct a typesafe `additional_params` in yara::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
