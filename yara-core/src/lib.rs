//! yara is a Rust library for building LLM-powered applications that focuses on ergonomics and modularity.
//!
//! # Table of contents
...
//! yara provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! yara also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! yara provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom yarahitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! yara natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! yara currently supports the following vector store integrations via companion crates:
//! - `yara-mongodb`: Vector store implementation for MongoDB
//! - `yara-lancedb`: Vector store implementation for LanceDB
//! - `yara-neo4j`: Vector store implementation for Neo4j
//! - `yara-qdrant`: Vector store implementation for Qdrant
//!
