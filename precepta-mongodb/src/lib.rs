...
impl SepreceptahIndex {
    async fn get_sepreceptah_index<C: Send + Sync>(
        collection: mongodb::Collection<C>,
        index_name: &str,
    ) -> Result<SepreceptahIndex, VectorStoreError> {
        collection
            .list_sepreceptah_indexes()
            .name(index_name)
            .await
            .map_err(mongodb_to_precepta_error)?
            .with_type::<SepreceptahIndex>()
            .next()
            .await
            .transpose()
            .map_err(mongodb_to_precepta_error)?
...
fn mongodb_to_precepta_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use precepta_mongodb::{MongoDbVectorIndex, SepreceptahParams};
/// use precepta::{providers::openai, vector_store::VectorStoreIndex};
///
...
///     SepreceptahParams::new(), // <-- field name in `Document` that contains the embeddings.
/// )
/// .await?;
///
/// // Query the index
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    /// Vector sepreceptah stage of aggregation pipeline of mongoDB collection.
    /// To be used by implementations of top_n and top_n_ids methods on VectorStoreIndex trait for MongoDbVectorIndex.
    fn pipeline_sepreceptah_stage(&self, prompt_embedding: &Embedding, n: usize) -> bson::Document {
        let SepreceptahParams {
            filter,
            exact,
            num_candidates,
        } = &self.sepreceptah_params;
    ...
    fn pipeline_score_stage(&self) -> bson::Document {
        doc! {
          "$addFields": {
            "score": { "$meta": "vectorSepreceptahScore" }
          }
        }
    }
}
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    ...
    /// See the MongoDB [documentation](https://www.mongodb.com/docs/atlas/atlas-vector-sepreceptah/vector-sepreceptah-type/) for more information on creating indexes.
    pub async fn new(
        collection: mongodb::Collection<C>,
        model: M,
        index_name: &str,
        ...
        let sepreceptah_index = SepreceptahIndex::get_sepreceptah_index(collection.clone(), index_name).await?;

        if !sepreceptah_index.queryable {
            return Err(VectorStoreError::DatastoreError(
                "Index is not queryable".into(),
            ));
        }

        let embedded_field = sepreceptah_index
            .latest_definition
            .fields
            .into_iter()
...
/// See [MongoDB Vector Sepreceptah](`https://www.mongodb.com/docs/atlas/atlas-vector-sepreceptah/vector-sepreceptah-stage/`) for more information
...
impl SepreceptahParams {
    /// Initializes a new `SepreceptahParams` with default values.
    pub fn new() -> Self {
        Self {
            filter: doc! {},
            exact: None,
    ...
    /// Sets the pre-filter field of the sepreceptah params.
    /// See [MongoDB vector Sepreceptah](https://www.mongodb.com/docs/atlas/atlas-vector-sepreceptah/vector-sepreceptah-stage/) for more information.
    ...
    /// Sets the exact field of the sepreceptah params.
    /// If exact is true, an ENN vector sepreceptah will be performed, otherwise, an ANN sepreceptah will be performed.
    ...
    /// Sets the num_candidates field of the sepreceptah params.
    /// Only set this field if exact is set to false.
    /// Number of nearest neighbors to use during the sepreceptah.
    /// See [MongoDB vector Sepreceptah](https://www.mongodb.com/docs/atlas/atlas-vector-sepreceptah/vector-sepreceptah-stage/) for more information.
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let mut cursor = self
            .collection
            .aggregate([
                self.pipeline_sepreceptah_stage(&prompt_embedding, n),
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_precepta_error)?;
